...
export function drawImageObject(
  ...
  if (selectedObjects.value.has(object.uuid)) {
    ...
    context.strokeRect(0, 0, imageSize.x, imageSize.y);
  }
}

export function drawStrokeObject(
  object: TabletopObject,
  context: CanvasRenderingContext2D
) {
  if (!object.strokes) {
  ...
  if (object.strokes && object.strokes.length > 1) {
    ...
    if (selectedObjects.value.has(object.uuid)) {
      ...
      context.strokeRect(min[0], min[1], max[0] - min[0], max[1] - min[1]);
    }
  }
}

export function drawTokenUi(
  object: TabletopObject,
  context: CanvasRenderingContext2D
) {
  context.save();
  ...
  context.translate((bounds[2] - bounds[0]) / 2, -30);
  ...
  context.translate((bounds[2] - bounds[0]) / 2, bounds[3] - bounds[1] + 30);
  ...
  context.translate(20, bounds[3] - bounds[1] - 20);

  // draw background
  context.beginPath();
  context.fillStyle = "#fb7185";
  context.roundRect(-20, -20, 40, 40, 10);
  context.fill();
  ...
  context.translate(bounds[2] - bounds[0] - 20, bounds[3] - bounds[1] - 20);
...
export function onUpdate(
  ...
  const canvasSizeHalf = new Vector2(canvas.width / 2, canvas.height / 2);
  context.translate(canvasSizeHalf.x, canvasSizeHalf.y);
  context.scale(camera.value.zoom, camera.value.zoom);
  context.translate(camera.value.position.x, camera.value.position.y);
