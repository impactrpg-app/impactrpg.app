...
export class PhysicsSystem extends Core.System {
  ...
  private _rigidBodyMap = new Map<
    string,
    InstanceType<typeof RAPIER.RigidBody>
  >();
  private _colliderMap = new Map<
    string,
    InstanceType<typeof RAPIER.Collider>
  >();
  ...
  override async onAttached(): Promise<void> {
    ...
    Core.engine.events.addEventListener(
      ...
      (entity: InstanceType<typeof Core.Entity>) =>
        this.onEntityTransformChanged(entity)
    );
    Core.engine.events.addEventListener(
  ...
  private onModuleAdded(module: InstanceType<typeof Core.Module>) {
    if (module.getModuleType() === "rigidbody") {
      ...
      const parent = this.findModuleOnParent<RigidBodyModule>(
        "rigidbody",
        module.entity
      );
      let parentBody = parent
        ? this._rigidBodyMap.get(parent.entity.uuid)
        : undefined;
